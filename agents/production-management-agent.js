...
class ProductionManagementAgent {
  ...
  formatScriptForTTS(script) {
    ...
    if (script.mainContent && script.mainContent.sections) {
      script.mainContent.sections.forEach((section) => {
        // Speak the section title itself as a natural transition — "Section N:"
        // was a structural label that ended up spoken aloud by the TTS engine.
        if (section.title) {
  ...
  createVisualPromptsFromScript(script) {
    ...
    // One image per section keeps slideshow pacing reasonable for longer
    // scripts; a flat cap of 5 left an 11-minute video with ~2.5 minutes of
    // static screen per image. Still capped for cost control.
